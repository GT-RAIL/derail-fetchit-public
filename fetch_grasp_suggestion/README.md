...
1. Launch the grasp suggestion collector:
   1. (Optional) If transform lookup error between "map" and "base_link" occurs, publish tf by running `rosrun tf static_transform_publisher 0 0 0 0 0 0 base_link map 100`
   1. Run MoveIt! on the fetch with: `roslaunch fetch_moveit_config move_group.launch allow_active_sensing:=true`
