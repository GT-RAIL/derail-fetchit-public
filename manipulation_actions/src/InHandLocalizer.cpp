    ...
    point_head_client("head_controller/point_head")
{
  ...
  pnh.param<double>("gear_pose_threshold", gear_pose_threshold, M_PI/12);
...
void InHandLocalizer::executeLocalize(const manipulation_actions::InHandLocalizeGoalConstPtr &goal)
{
  ...
  if (goal->correct_object_direction)
  {
    ...
    double object_angle = acos(object_x_vector.dot(gripper_x_vector));

    if (object_angle > gear_pose_threshold && object_angle < M_PI - gear_pose_threshold)
    {
