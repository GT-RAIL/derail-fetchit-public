...
def detect(req, net):
    # prototxt = os.path.join(rospkg.RosPack().get_path('rail_part_affordance_detection'),
    #                         'lib/affordance_net/models/pascal_voc/VGG16/faster_rcnn_end2end/test.prototxt')
    # caffemodel = os.path.join(rospkg.RosPack().get_path('rail_part_affordance_detection'),
    #                           'lib/affordance_net/pretrained/AffordanceNet_200K.caffemodel')
    #
...
if __name__ == '__main__':
    ...
    DETECT_AFFORDANCES_SERVICE_NAME = "rail_part_affordance_detection/detect_affordances"
    server = rospy.Service(DETECT_AFFORDANCES_SERVICE_NAME, DetectAffordances,
                           lambda msg: detect(msg, net))
